...
export default function Hero() {
  return (
    <ScrollReveal>
      <section
        aria-label="Hero"
        className="relative flex min-h-screen items-center overflow-hidden bg-gradient-to-b from-[var(--color-cream-100)] via-[var(--color-bg-primary)] to-[var(--color-cream-50)] dark:from-[var(--color-espresso-900)] dark:via-[var(--color-espresso-800)] dark:to-[var(--color-espresso-900)]"
      >
        ...
        <div className="pointer-events-none absolute -top-1/2 right-0 h-[120%] w-1/2 bg-[radial-gradient(ellipse_80%_60%_at_50%_40%,rgba(196,149,106,0.10)_0%,transparent_60%)] dark:bg-[radial-gradient(ellipse_80%_60%_at_50%_40%,rgba(212,168,122,0.08)_0%,transparent_60%)]" />
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-7xl grid-cols-1 gap-8 px-6 sm:px-8 lg:grid-cols-12 lg:px-12">
          ...
          {/* Right column — decorative accent (desktop only) */}
          <div className="hidden items-center justify-center lg:col-span-3 lg:flex">
            <div className="h-48 w-px bg-gradient-to-b from-transparent via-[var(--color-accent)] to-transparent opacity-40" />
          </div>
        </div>
